...
if __name__ == '__main__':
    ...
    smoothed1=smooth("makespan1.csv",weight=0.989)
    smoothed1=xiugai1(smoothed1)
    plt.plot(range(0, len(smoothed1)), smoothed1, 'orange',linewidth=1)
    smoothed2=smooth("makespan2.csv",weight=0.99)
    smoothed2=xiugai2(smoothed2)
    plt.plot(range(0, len(smoothed2)), smoothed2, 'blue',linewidth=1)
    smoothed3 = smooth("makespan3.csv", weight=0.988)
    smoothed3 = xiugai2(smoothed3)
    plt.plot(range(0, len(smoothed3)), smoothed3, 'maroon', linewidth=1)
    plt.plot(range(0,1000),[3600 for i in range(0,1000)],'green',linewidth=1)
    plt.plot(range(0, 1000), [3610 for i in range(0, 1000)], 'red', linewidth=1)
    plt.plot(range(0,1000),[3740 for i in range(0,1000)],'purple',linewidth=1)
    plt.plot(range(0,1000),[3682 for i in range(0,1000)],'deepskyblue',linewidth=1)
    plt.plot(range(0,1000),[3660 for i in range(0,1000)],'brown',linewidth=1)
    plt.plot(range(0,1000),[3837 for i in range(0,1000)],'deeppink',linewidth=1)
    #plt.plot(range(0,1000),[3717 for i in range(0,1000)],'gold',linewidth=1)
    plt.legend(['LSTM-PPO', "GA","DQN",'FIFO','LIFO','SPT','LPT','SRPT','LRPT'])
    plt.xlabel('training times', fontsize=12)
    plt.ylabel('maximum completion time(s)', fontsize=12)
    plt.show()
    ['blue', 'brown', 'orange', 'red', 'purple', 'yellowgreen', 'deeppink', 'deepskyblue', 'lightseagreen',
     'gold','green','maroon','black']
